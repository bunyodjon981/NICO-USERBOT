...
# ═══════════════════════════════════════════════════════════════
#  ⑦ USERBOT HANDLERLAR — BARCHA BUYRUQLAR + ANIMATSIYALAR
#     (Asl fayldan to'liq nusxa, xuddi shu ko'rinishda)
# ═══════════════════════════════════════════════════════════════
def _register_ub_handlers(cl: TelegramClient):
    ...
    async def help_cmd(event):
        await event.edit(
            ...
            "• `.adminai on/off` - Barcha lichkalarga AI javob beradi\n"
            "• `.adminai hazil` - Admin ai hazil so'z aytadi\n"
            "• `.cods [ matn ]` - AI kod yozib beradi\n"
            "• `.ai [matn]` - O'zingiz uchun AI javobi\n"
            "• `.admin_prompt` - AI aqlliligini sozlash\n"
    ...
    async def ping_cmd(event):
        ...
        await event.edit(f"🛰 **PROFESSIONAL USERBOT Tezligi:**\n\n⚡️ **Ping:** `{ms} ms`\n📊 **Holat:** {status}")
    ...
    async def user_info(event):
        ...
        await event.edit("🔍 **Admin AI: Foydalanuvchi tahlil qilinmoqda...**")
    ...
    async def universal_saver(event):
        ...
        reply_msg = await event.get_reply_message()
        await event.edit("🔄 **Userbot: Ma'lumot xotiraga ko'chirilmoqda...**")
        try:
    ...
    async def admin_status(event):
        ...
        await event.edit(
            ...
            f"🚀 *NICO USERBOT v1.0 barqaror ishlamoqda!*"
        )
    ...
    async def ai_coder(event):
        ...
        await event.edit("🤖 **Titan AI o'ylamoqda...**")
        try:
            ...
            if response:
                final_text = f"💻 **Titan AI tomonidan yozilgan kod:**\n\n<code>{response}</code>"
    ...
    # ── .adminai ──
    @cl.on(events.NewMessage(outgoing=True, pattern=r'\.adminai (on|off|hazil)'))
    async def toggle_ai(event):
        me   = await cl.get_me()
        cfg  = get_ub_cfg(me.id)
        mode = event.pattern_match.group(1)
        if mode == "hazil":
            await event.edit("🤡 **Admin AI hazil qilmoqchi...**")
            ...
            await event.edit(f"🤖 **Admin AI tizimi {status}**")

    # ── .admin_prompt ──
    @cl.on(events.NewMessage(outgoing=True, pattern=r'\.admin_prompt'))
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(api_url, timeout=aiohttp.ClientTimeout(total=30)) as response:
                    if response.status == 200:
                        ...
                        image_file.name = "titan_ai_image.png"
                        await event.delete()
                        await cl.send_file(
    ...
    async def auto_group_creator(event):
        ...
        for i in range(1, count + 1):
            try:
                group_title = f"Professional Group {year} - №{i}"
    ...
    async def func_menu(event):
        await event.edit(
            "🎭 **PROFESSIONAL USERBOT: ANIMATSIYALAR** 🎭\n"
    ...
    async def charge_anim(event):
        for frame in ["🔋 [          ] 0%","🔋 [▬▬        ] 20%","🔋 [▬▬▬      ] 40%","🔋 [▬▬▬▬▬    ] 60%","🔋 [▬▬▬▬▬▬▬  ] 80%","🔋 [▬▬▬▬▬▬▬▬▬▬] 100%","⚡️ **NICO USERBOT to'liq quvvatda!**"]:
            await event.edit(f"<code>{frame}</code>", parse_mode='html')
            await asyncio.sleep(0.4)
    ...
    async def duel_anim(event):
        for frame in ["⚔️ ( •_•)        (•_• )","⚔️ ( •_•)>      (•_• )","⚔️      ⚔️(•_•) (•_• )","⚔️      ( •_•)⚔️(•_• )","⚔️      ( •_•)  (✖️_✖️)","🏆 **NICO USERBOT g'alaba qozondi!**"]:
            await event.edit(f"<code>{frame}</code>", parse_mode='html')
            await asyncio.sleep(0.5)
    ...
    async def fake_error(event):
        for frame in ["⚠️ <b>System Error!</b>","❌ <code>Critical: Memory Leak</code>",
                      ...
                      "🛑 <code>Rebooting in 1...</code>","😎 **Hazillashdim! NICO USERBOT doim onlayn!**"]:
            await event.edit(frame, parse_mode='html')
            await asyncio.sleep(0.7)

    @cl.on(events.NewMessage(outgoing=True, pattern=r'\.calc_pro'))
    async def calc_animation(event):
        for formula in ["x² + y² = z²","E = mc²","∫(f(x)) dx","lim(x→∞)","✅ **Yechim: NICO USERBOT V25**"]:
    ...
    async def galaxy_anim(event):
        ...
        for _ in range(2):
            for frame in frames:
                await event.edit(f"{frame} **NICO galaktika bo'ylab...** {frame}")
                await asyncio.sleep(0.4)
    ...
    async def virus_anim(event):
        for frame in ["⚠️ **DIQQAT: Tizimda virus aniqlandi!**","🔴 **Xavf darajasi: Yuqori**",
                      ...
                      "💀 **Siz xakerlar hujumiga uchradingiz!**","😜 **Hazillashdim, bu shunchaki NICO USERBOT!**"]:
            await event.edit(frame)
            await asyncio.sleep(0.6)
    ...
    async def fight_anim(event):
        for frame in ["🤺      🤺","🤺    🤺","🤺  🤺","🤺🤺","🥊💥🤺","🥇 **NICO USERBOT g'olib!**"]:
            await event.edit(frame)
            await asyncio.sleep(0.4)
    ...
    async def dragon_anim(event):
        for frame in ["🐉","🐉☁️","🐉☁️🔥","🐉☁️🔥🔥","🐉🔥🔥🔥","🐲🔥🔥🔥🔥","🐲💥💥💥","💨💨💨","💀 **NICO dushmanlarni kul qildi!**"]:
            await event.edit(frame)
            await asyncio.sleep(0.4)
    ...
    async def earth_anim(event):
        ...
        for _ in range(3):
            for frame in frames:
                await event.edit(f"{frame} **NICO kosmosda...**")
                await asyncio.sleep(0.3)
    ...
    async def load_anim(event):
        ...
        for bar, percent in frames:
            try:
                await event.edit(f"📥 **NICO: Yuklash jarayoni**\n\n`[{bar}]` **{percent}**")
                await asyncio.sleep(0.4)
            except Exception:
                break
        await asyncio.sleep(0.5)
...
# Ruxsat etilgan foydalanuvchi tugmalari
USER_BTNS = [
    "🛒 Kod sotib olish", "📞 Murojat qilish",
    "⏸ Userbotni to'xtatish", "▶️ Userbotni yoqish",
    "📱 Telefon raqamni ulash"
]
# Ruxsat etilgan admin tugmalari
ADMIN_BTNS = [
    "📦 Kod yuklash", "💳 To'lov tizimlari",
    "📢 Xabar yuborish", "💲 Narxni tahrirlash",
    "👥 Foydalanuvchilar", "🔄 Restart",
    "👑 Boshqaruv bo'limi", "⬅️ Orqaga"
]
# Admin step holatlari
ADMIN_STEPS = (
    ...
    "broadcast","admin_reply_to","plugin_bot","plugin_ub"
)
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── ADMIN ──
    if is_admin(uid):
        # Admin step ichida bo'lsa — _admin_text ga yuborish
        if step in ADMIN_STEPS:
            await _admin_text(update, ctx, text)
            return
        # Admin bosh menyu tugmalari
        if "Boshqaruv" in text:
            await _show_admin_menu(update)
            return
        if any(k in text for k in ADMIN_BTNS):
            await _admin_text(update, ctx, text)
            return
        # Admin user tugmalarini ham ishlatishi mumkin
        # (kod sotib olish, murojat, userbot ulash va h.k.)
        # Qolgan barcha narsalar — user sifatida ishlaydi

    # ── BARCHA FOYDALANUVCHILAR (admin ham kiradi) ──
    ...
    if step == "wait_receipt":
        # Matn kelsa rad etish — faqat rasm/fayl kerak
        await update.message.reply_text(
            "❌ <b>Faqat chek rasmi qabul qilinadi!</b>\n\n"
            "📸 Iltimos, to'lov chekini <b>rasm sifatida</b> yuboring.",
            parse_mode=ParseMode.HTML
        )
        return
    ...
    if "Kod sotib olish" in text:
        ...
        # Noma'lum matn — xato xabari
        if step == "":
            await update.message.reply_text(
                "⚠️ <b>Bunday buyruq mavjud emas.</b>\n\n"
                "Pastdagi menyulardan foydalaning 👇",
                parse_mode=ParseMode.HTML)
        # Aks holda — bosh menyu ko'rsat
        await _show_user_main(update, uid)
...
async def _finish_login(update, ctx, uid, cl):
    ...
    await update.message.reply_text(
        f"✅ <b>Userbot muvaffaqiyatli ulandi!</b>\n\n"
...
async def cb_pay_card(update, ctx):
    ...
    markup = InlineKeyboardMarkup([[InlineKeyboardButton("❌ Bekor qilish", callback_data="cancel_receipt")]])
...
async def handle_receipt_photo(update, ctx):
    """Rasm yuborganda chek qabul qilish"""
    uid = update.effective_user.id
    if is_admin(uid):
        return
    step = STATES.get(uid, {}).get("step", "")
    if step != "wait_receipt":
        return
    await _process_chek(update, ctx, uid)


async def handle_receipt_doc(update, ctx):
    """Fayl yuborganda chek qabul qilish"""
    uid = update.effective_user.id
    if is_admin(uid):
        return
    step = STATES.get(uid, {}).get("step", "")
...
async def _process_chek(update, ctx, uid):
    """Chekni adminga yuborish — rasm, fayl yoki matn"""
    u  = users().get(str(uid), {})
    p  = prod()
    markup = InlineKeyboardMarkup([[
    ...
    try:
        if msg.photo:
            ...
            # Fayl (rasm fayl ko'rinishida) — qabul qilinadi
            ...
            # Matn yoki boshqa narsa — rad etiladi
            await msg.reply_text(
                "❌ <b>Faqat chek rasmi qabul qilinadi!</b>\n\n"
                "📸 Iltimos, to'lov chekini <b>rasm sifatida</b> yuboring.",
                parse_mode=ParseMode.HTML
        ...
        await msg.reply_text(f"❌ Yuborishda xato: {e}")
        return
...
async def _receipt_text_to_admin(update, ctx, uid, text):
    # Matn chek endi qabul qilinmaydi — faqat rasm
    await update.message.reply_text(
        "❌ Iltimos, faqat <b>chek rasmini</b> yuboring!\n"
        "Matn qabul qilinmaydi.",
        parse_mode=ParseMode.HTML
    )
    return
async def _receipt_text_to_admin_DISABLED(update, ctx, uid, text):
    u  = users().get(str(uid), {})
    p  = prod()
    markup = InlineKeyboardMarkup([[
        InlineKeyboardButton("✅ Tasdiqlash", callback_data=f"approve_{uid}"),
...
async def _admin_text(update, ctx, text):
    ...
    if step == "plugin_bot":
        await _save_plugin(update, ctx, text, "bot")
        return

    if step == "plugin_ub":
        await _save_plugin(update, ctx, text, "ub")
        return
...
async def _admin_btn(update, ctx, text):
    ...
    elif "Orqaga" in text or "⬅️" in text:
        await _show_user_main(update, uid)
    elif "Restart" in text:
        await update.message.reply_text(
            "♻️ <b>NICO USERBOT qayta ishga tushirilmoqda...</b>\n"
            "Barcha ma'lumotlar o'chiriladi.",
            parse_mode=ParseMode.HTML)
        # Barcha userbot sessionlarni yopish
        for cl in list(UB.values()):
            try:
                await cl.disconnect()
            except Exception:
                pass
...
# ═══════════════════════════════════════════════════════════════
#  ⑮ PLUGIN SAQLASH FUNKSIYASI
# ═══════════════════════════════════════════════════════════════
async def _save_plugin(update, ctx, code_text: str, plugin_type: str):
    """Plugin kodini saqlash va restart"""
    uid = ADMIN_ID
    # Papka yaratish
    folder = f"plugins/{plugin_type}"
    os.makedirs(folder, exist_ok=True)

    # Fayl nomi — vaqt bilan
    ts    = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    fname = f"{folder}/plugin_{ts}.py"

    # Kodni saqlash
    with open(fname, "w", encoding="utf-8") as f:
        f.write(code_text)

    tip = "🤖 Bot" if plugin_type == "bot" else "👤 Userbot"
    await update.message.reply_text(
        f"✅ <b>{tip} plugin saqlandi!</b>\n\n"
        f"📄 Fayl: <code>{fname}</code>\n"
        f"📏 Hajm: {len(code_text):,} belgi\n\n"
        f"♻️ Bot qayta ishga tushirilmoqda...",
        parse_mode=ParseMode.HTML
    )
    STATES.pop(uid, None)
    db_save()
...
def _load_plugins(cl_for_ub=None):
    """Barcha pluginlarni yuklash"""
    # Bot pluginlari
    bot_dir = "plugins/bot"
            ...
            if fname.endswith(".py"):
                ...
                try:
                    with open(fpath, encoding="utf-8") as f:
                        code = f.read()
                    exec(compile(code, fpath, "exec"), {"asyncio": asyncio, "os": os})
                    logging.warning(f"✅ Bot plugin yuklandi: {fname}")
                except Exception as e:
                    logging.warning(f"❌ Bot plugin xato {fname}: {e}")

    # Userbot pluginlari
    if cl_for_ub:
        ub_dir = "plugins/ub"
        if os.path.isdir(ub_dir):
            for fname in sorted(os.listdir(ub_dir)):
                if fname.endswith(".py"):
                    fpath = os.path.join(ub_dir, fname)
                    try:
                        with open(fpath, encoding="utf-8") as f:
                            code = f.read()
                        exec(compile(code, fpath, "exec"),
                             {"cl": cl_for_ub, "events": events,
                              "asyncio": asyncio, "os": os})
                        logging.warning(f"✅ UB plugin yuklandi: {fname}")
                    except Exception as e:
                        logging.warning(f"❌ UB plugin xato {fname}: {e}")


# ═══════════════════════════════════════════════════════════════
#  ⑮ .py FAYL QABUL QILISH
# ═══════════════════════════════════════════════════════════════
async def handle_doc_universal(update, ctx):
    """Fayl handler — admin uchun kod yuklash, foydalanuvchi uchun chek"""
    uid  = update.effective_user.id

    # Foydalanuvchi wait_receipt stepida — chek sifatida qabul qilish
    if not is_admin(uid):
        step = STATES.get(uid, {}).get("step", "")
        if step == "wait_receipt":
            await _process_chek(update, ctx, uid)
...
async def callback_handler(update, ctx):
    ...
    if data == "buy_code":
        ...
        ub_active   = uid_c in UB
        has_session = bool(sessions().get(str(uid_c), {}).get("session"))
        rows = [[KeyboardButton("🛒 Kod sotib olish"), KeyboardButton("📞 Murojat qilish")]]
        if ub_active:
            rows.append([KeyboardButton("⏸ Userbotni to'xtatish")])
        elif has_session:
        ...
        await query.answer("📸 Chek rasmini yuboring.", show_alert=True)
...
async def main():
    ...
    # Chek uchun — rasm, fayl, matn barchasi
    app.add_handler(MessageHandler(filters.PHOTO & ~filters.COMMAND, handle_receipt_photo))

    # Fayl — admin uchun kod yuklash YOKI foydalanuvchi chek sifatida
    app.add_handler(MessageHandler(filters.Document.ALL & ~filters.COMMAND, handle_doc_universal))
